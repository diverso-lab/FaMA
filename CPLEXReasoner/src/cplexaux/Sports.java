...
public class Sports {
    ...
    private void distancePref() throws IloException{
    	IloCP cp = new IloCP();
    	int aroundValue = 0, upperBound = 0, lowerBound = 100, maxRange = 80;
    	IloNumVar var = cp.numVar(0, 100);
    	
    	
    	cp.quot(cp.min(maxRange,cp.abs(cp.min(var, aroundValue))),maxRange);
    }
    
    private void LikesPref() throws IloException{
    	IloCP cp = new IloCP();
    	
    	cp.eq(cp.boolVar(), 1);
    }
    
    private void aggregatePrefs(IloNumExpr... exprs) throws IloException{
    	IloCP cp = new IloCP();
    	cp.sum(exprs);
    }
}
