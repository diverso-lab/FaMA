...
public abstract class DefaultValidAttributedConfigurationQuestion implements
	...
	public PerformanceResult answer(Reasoner r) {
		...
		if (p == null) {
			...
			Collection<GenericAttributedFeature> prodFeats = this.p
					.getAttFeatures();
			Collection<? extends GenericAttributedFeature> excludeFeats = this
					.getAllFeatures();

			// if prodFeats contains features that are not on the Config
			// Model,
			// the parcial product is not valid

			if (!excludeFeats.containsAll(prodFeats)) {
				valid = false;
				System.err.println("Those feature are not part of the model");
				Iterator<GenericAttributedFeature> featsIt = prodFeats
						.iterator();
				...
				Collection<GenericAttributedFeature> excludeFeats2 = new ArrayList<GenericAttributedFeature>(
						excludeFeats);
				...
				while (it1.hasNext()) {
					...
					conf.addElement(f, 1);
//					Collection<? extends GenericAttribute> atts = f
//							.getAttributes();
//					Iterator<? extends GenericAttribute> itAtts = atts
//							.iterator();
					// =======================================
					// Los atributos no han de tener implicacion en la
					// configuracion, ya que no implican una configuracion err
					// onea,es decir hasta que el producto no este acabado no
					// podemos saber
					// si los atributos que se han marcado son erroneos
					// =======================================
					// while (itAtts.hasNext()){
					// IntegerAttribute a = (IntegerAttribute)itAtts.next();
					...
					// (estoy filtrandolo)
					// fq.addValue(a, a.getValue());
					// }
					// }
				}

				Iterator<GenericAttributedFeature> it2 = excludeFeats2
						.iterator();
				while (it2.hasNext()) {
