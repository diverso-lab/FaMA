...
package es.us.isa.FAMA.Benchmarking.CSV;
import java.io.*;

/**
 * Print values as a comma separated list that can be read by the Excel spreadsheet. More information about this class is available from <a target="_top" href= "http://ostermiller.org/utils/ExcelCSV.html">ostermiller.org</a>.
			...
			if (args.length > 0){
				...
				if (!f.exists()){
					...
					if (f.canWrite()){
						...
						throw new IOException("Could not open test.csv");
					}
				} else {
					throw new IOException("File already exists: test.csv");
				}
			} else {
				out = System.out;
			}
