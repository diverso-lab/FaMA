...
public class JavaBDDReasoner extends FeatureModelReasoner {
	...
	public void addConstraint(Constraint c) {
		BDD result = parser.translateToConstraint(c.getAST());
		subtrees.add(result);
	}
	
	protected class BDDParser {
...
//			featName = null;
		...
		public BDD translateToConstraint(Tree<String> ast) {
//			featName = null;
			BDD res = null;
		...
		private BDD translateLogical(Node<String> tree) {
			// constraints logicas:
			...
			if (n == 2) {
				if (data.equals(KeyWords.AND)) {
					BDD e1 = translateLogical(children.get(0));
					BDD e2 = translateLogical(children.get(1));
					res = e1.apply(e2, BDDFactory.and);
					//BDDFactory.and
				} else if (data.equals(KeyWords.OR)) {
					BDD e1 = translateLogical(children.get(0));
					BDD e2 = translateLogical(children.get(1));
					res = e1.apply(e2, BDDFactory.or);
					//BDDFactory.or
				} else if (data.equals(KeyWords.IMPLIES)
						|| data.equals(KeyWords.REQUIRES)) {
					BDD e1 = translateLogical(children.get(0));
					BDD e2 = translateLogical(children.get(1));
					res = e1.apply(e2, BDDFactory.imp);
					//BDDFactory.imp
				} else if (data.equals(KeyWords.IFF)) {
					BDD e1 = translateLogical(children.get(0));
					BDD e2 = translateLogical(children.get(1));
					res = e1.apply(e2, BDDFactory.biimp);
					//BDDFactory.biimp
					...
					BDD e1 = translateLogical(children.get(0));
					BDD aux = translateLogical(children.get(1));
					BDD e2 = aux.not();
					res = e1.apply(e2, BDDFactory.imp);
				} else {
				...
				if (data.equals(KeyWords.NOT)) {
					BDD e1 = translateLogical(children.get(0));
					res = e1.not();
//					BDD zero = ((BDDFactory) getBDDFactory()).zero();
//					res = e1.apply(zero,
//							BDDFactory.biimp);
				}
				else {
				...
				if (isFeature(tree)) {
					res = variables.get(data);
//					BDD feat = variables.get(data);
//					BDD one = ((BDDFactory) getBDDFactory()).one();
//					res = feat.apply(one,
//							BDDFactory.biimp);
				}
