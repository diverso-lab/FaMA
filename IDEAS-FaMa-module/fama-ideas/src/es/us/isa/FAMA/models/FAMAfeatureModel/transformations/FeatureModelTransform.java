...
public class FeatureModelTransform implements ITransform {
	...
	public void update() {
		...
		// ��Que intencion tiene este m��todo?
	}
	...
	private void generateConstraints(FAMAFeatureModel fm) {
		...
		Iterator<Constraint> it = fm.getDependencies();
		while (it.hasNext()) {
			Constraint c = it.next();
			if (c instanceof RequiresDependency){
				RequiresDependency dep = (RequiresDependency) c;
				r.addRequires(dep, dep.getOrigin(), dep.getDestination());
			}	
			else if (c instanceof ExcludesDependency){
				ExcludesDependency dep = (ExcludesDependency) c;
