...
entrada returns [Order result = new Order();]
{Configuration aux = null; ExtendedConfiguration orderConf = null;}: 
#(ORDER (orderConf = order_config {result.setOrderConfiguration(orderConf);})
 (aux = config{result.addConfiguration(aux);})+ );

order_config returns [ExtendedConfiguration res = new ExtendedConfiguration();]
{Collection<Tree<String>> aux = null;}:
(aux = constraints {res.setAttConfigs(aux);}); 

config returns [Configuration res = null;] 
